  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-5 grid-col gap-5 mb-5 shadow-sm p-3 drop-shadow-sm rounded-md shadow-slate-600">
      ...
      <div className="col-span-2 flex flex-col gap-3 items-start">
        <div className="w-full flex justify-between items-center my-1 gap-6">
          <h2 className="text-2xl overflow-clip">{item.title}</h2>
          <Link to={`/post/${item.id}`} title="Share">
            <ArrowUpRightSquare className="text-gray-500 cursor-pointer w-6 h-6" />
          </Link>
        </div>
        <p className="max-h-[310px] text-base overflow-clip capitalize">
          {item.summary}
        </p>
        <div
          className="content max-h-[70px] text-md text-gray-400 overflow-clip"
